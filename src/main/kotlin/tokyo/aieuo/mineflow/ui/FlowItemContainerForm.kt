...
object FlowItemContainerForm {
    ...
    fun sendMoveAction(player: Player, container: FlowItemContainer, type: String, selected: Int, messages: List<String> = listOf(), count: Int = 0) {
        val actions = container.getItems(type)
        ...
        val buttons = mutableListOf(
            Button("@form.back") {
                FlowItemForm.sendAddedItemMenu(player, container, type, actions[selected], listOf(if (count == 0) "@form.cancelled" else "@form.moved"))
            },
        )
        ...
        for ((i, action) in actions.withIndex()) {
            ...
            buttons.add(Button(color + TextFormat.clean(action.getDetail()).trim()) {
                if (i == selected || action !is FlowItemContainer) {
                    ...
                    sendMoveAction(player, action, type, action.getItems(type).size - 1, listOf("@form.moved"), count + 1)
                }
            })
        }
        ...
        (ListForm(Language.get("form.${type}Container.move.title", listOf(container.getContainerName(), selectedAction.getName()))))
    ...
    fun moveContent(player: Player, container: FlowItemContainer, type: String, actions: List<FlowItem>, from: Int, to: Int, count: Int) {
